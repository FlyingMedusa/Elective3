...
esop = esop.split('  ') #bo są dyphtongi

vow = []
cons = []
for word in esop:
    for sound in word.split():
        if sound not in consmap:
            vow.append(sound)
        else:
            cons.append(sound)


len_v = len([c for c in cons if consmap[c][2] == 1])
len_vless = len([d for d in cons if consmap[d][2] == 0])


#zakładam, że vowels to też voiced phonemes
print("The text 'esop' contains", (len_v + len(vow)), "voiced phonemes and", len_vless, "voiceless ones, the voiced phonemes therefore constitute", str(eval(str(round((len_v+len(vow))/(len(vow)+len(cons))*100,1)))) + "%.")
print("In the text, there are", len(vow), "vowels and", len(cons), "consonants.")